...
class book
{
    ...
    public function database()
    {
        ...
        try {
            $sth = $dns->exec('USE librairie;INSERT INTO LIBRARY(BOOK_ID,USER_ID,IMAGE,TITRE,AUTEUR,LU) VALUES("' . $this->book_id . '",' . $this->user_id . ',"' . $this->img . '","' . $this->titre . '","' . $this->auteur . '",' . $lu . ');');
        } catch (Exception $e) {
            echo "erreur db", $e->getMessage();
        }
    }
    ...
    public static function BookLu($book_id, $lu)
    {
        ...
        try {
            $sth = $dns->exec('USE librairie; UPDATE LIBRARY SET LU = ' . $lu . ' WHERE BOOK_ID = "' . $book_id . '";');
        } catch (Exception $e) {
            echo "erreur db", $e->getMessage();
        }
    }
    ...
    public static function BookSup($book_id,$user_id){
        ...
        try {
            $sth = $dns->exec('USE librairie; DELETE FROM LIBRARY WHERE BOOK_ID = "'.$book_id.'" AND USER_ID = '.$user_id.';');
        } catch (Exception $e) {
            echo "erreur db", $e->getMessage();
        }
    }
